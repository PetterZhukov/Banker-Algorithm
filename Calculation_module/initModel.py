...
class initData:
    ...
    def getAllocatedList(self):
        "已分配矩阵"
        return [[retAllocatedResources()if self.boolNeed[i][j] else 0 for j in range(self.resoNum)] for i in range(self.clientNum)]

    def getTimeList(self):
        "所用时间矩阵"
        return [[retTime() if self.boolNeed[i][j] else 0 for j in range(self.resoNum)] for i in range(self.clientNum)]

    def getNeedList(self):
        "仍然需求资源矩阵"
        return [[retStillNeedResources()if self.boolNeed[i][j] else 0 for j in range(self.resoNum)] for i in range(self.clientNum)]
